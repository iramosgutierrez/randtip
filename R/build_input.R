...
check.info<- function(info, tree, sim=0.8){
  ...
  for (rank in ranks){
    ...
    for(group in groups){
      phyle.type<- MDCC.phyleticity(info, tree,
                                    MDCC.info = list("rank"= rank, "MDCC"= group))
      DF[which(DF[,rank]==group), paste0(rank,"_phyletic.status")]<-phyle.type
  ...
  if(length(tips[duplicated(tips)])>=1){
    message("Tips ", tips[duplicated(tips)],
            " are duplicated in the phylogeny tips. Please remove one of them.\n")
  }
