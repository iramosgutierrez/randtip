...
#' @export
add.to.polyphyletic <- function(tree, species, polyphyletic.insertion = c("freq", "large", "all")){
    
    species <- gsub(" ", "_", species)
    genus <- unique(stringr::word(species, 1, sep = "_"))
    if(length(genus) > 1){
    ...
    if(polyphyletic.insertion == "all"){
        for(sp in species){
            ...
            new.tree <- add.into.node(new.tree, node = mrca, new.tip = sp)
        }
    ...
    if(polyphyletic.insertion == "freq"){
        for(sticked.species in species){
            new.tree.sp <- new.tree$tip.label
    ...
    if(polyphyletic.insertion == "large"){
        ...
        for(sp in species){
            if(group.type == "monophyletic"){
                ...
                new.tree <- add.into.node(tree = new.tree, node = mrca, 
                                          new.tip = sp)}
